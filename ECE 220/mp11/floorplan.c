...
/* This MP detects, creates and converts a tree data structure by various functions that I wrote.
 * Although not as tedious as the previous MP, I still hate this MP.
 * Partner: hz13, yifuguo3, nayund2
 */
...
int is_leaf_node(node_t* ptr) {
  ...
  /* If the pointer does not have a right or left child, it is a leaf node. */
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  /* If current node is not a leaf node, then it is an internal node. */
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  if (is_internal_node(a)) return (is_in_subtree(a -> right, b) || is_in_subtree(a -> left, b));
  return 0;
}
...
void rotate(node_t* ptr) {
  // TODO:
  int temp;
  /* This part is simple enough that I don't think that I really need to explain... */
  temp = ptr -> module -> w;
...
void recut(node_t* ptr) {
  ...
  // TODO:
  /* If the original cut is V, then change the cut to H. */
  if (ptr -> cutline == V) ptr -> cutline = H;
  /* Otherwise, change the cut to V. */
  else if (ptr -> cutline == H) ptr -> cutline = V;
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  module_t* temp_mod;
  /* Switch modules by exchanging pointers. */
  temp_mod = a -> module;
  a -> module = b -> module;
  b -> module = temp_mod;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  assert(a->parent != NULL && b->parent != NULL);
    ...
    /* declare two variable to hold the parent of a and b */
    node_t *a_parent = a->parent;
    node_t *b_parent = b->parent;
    /* see whether a is the left or the right node of its parent*/
    if(a->parent->left == a && b -> parent -> left == b){
        /* swap a and b and change the parent of a and b */
        a_parent -> left = b;
        b_parent -> left = a;
        b -> parent = a_parent;
        a -> parent = b_parent;
    ...
    else if (a -> parent -> left == a && b -> parent -> right){
        /* swap a and b and change the parents of a and b */
        a_parent -> left = b;
        b_parent -> right = a;
        b -> parent = a_parent;
        a -> parent = b_parent;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  /* Base case ... */
  if (!ptr) return;
  /* Traverse the left subtree by recursively calling the Postfix. */
  postfix_traversal(ptr -> left, nth, expression);
  /* Traverse the right subtree by recursively calling the Postfix. */
...
/*
 * This is the helper function that I wrote. This function builds the tree and has no return value so it
 * won't mess up with the parent pointer.
 */
void build_tree(node_t* par, int n){


  /* malloc the current node. */
  /* Base case: build the bottom nodes. */
  if (n == num_modules - 1){
    module_t* m_ptr = (module_t*) malloc(sizeof(module_t));
  ...
  /* Place left child (future parent node). */
  node_t* left_ptr = (node_t*) malloc(sizeof(node_t));
  par -> left = left_ptr;
  left_ptr -> parent = par;
}
  /* Recursively call the build_tree function. */
  build_tree(par -> left, n+1);
}
