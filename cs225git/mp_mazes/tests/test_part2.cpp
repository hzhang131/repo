...
void assert_maze_acyclic(SquareMaze & maze, int width, int height)
{
	...
	REQUIRE(calls + components == width * height * 2);
...
{
	SquareMaze maze;
	maze.makeMaze(200, 200);
	assert_maze_connected(maze, 200, 200);
	PNG* output = maze.drawMaze();
	output -> writeToFile("myoutput"+ string(".png"));
