...
class KDTree
{
  ...
  private:
    ...
    void quickselect (vector<Point<Dim>> &, int, int, int, int);
    int partition (vector<Point<Dim>>&, int, int, int);
    KDTreeNode* Build_tree(vector<Point<Dim>> &, int, int, int);
    KDTreeNode* findNearestNeighborHelper(const Point<Dim> &, KDTreeNode*, int) const;
    double getDist(const Point<Dim>, const Point<Dim> ) const;
    KDTreeNode* getRoot(){
      return root;
    }
    size_t getSize(){
    ...
    void del(KDTreeNode* root);
    KDTreeNode* clone(KDTreeNode* other);
    /**
     * @todo Add your helper functions here.
     */
     //KDTreeNode* findNearestNeighbor_helper(const Point<Dim>& query, const KDTreeNode *subroot, int start_dim) const;
