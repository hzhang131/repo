...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while (singleQueue.size() + mergeQueue.size() > 1){
       ...
       TreeNode* new_node = new TreeNode((smaller -> freq.getFrequency()) + (smallest -> freq.getFrequency()));
       new_node -> left = smallest;
       new_node -> right = smaller;
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        if (next_bit) current = current -> right;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
    TreeNode* some_node = nullptr;
    if (!bfile.hasBits()) return some_node;
    bool next_bit = bfile.getNextBit();
    if (next_bit == true){
      some_node = new TreeNode(0, bfile.getNextByte());
      //some_node -> freq = new Frequency(bfile.getNextByte(),0);
      return some_node;
    }
    some_node = new TreeNode(0);
    some_node -> left = readTree(bfile);
    some_node -> right = readTree(bfile);
    return some_node;
}
