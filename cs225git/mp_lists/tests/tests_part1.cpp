...
TEST_CASE("List::split nothing 1", "[weight=5][part=1][valgrind]") {
    List<int> list;
...
TEST_CASE("List::split nothing 2", "[weight=5][part=1][valgrind]") {
    List<int> list;
...
TEST_CASE("List::split out of bound", "[weight=5][part=1][valgrind]") {
    List<int> list;
...
TEST_CASE("List::ListIterator::operator-- (post-decrement) returns an un-incremented iterator", "[weight=1][part=1][valgrind]") {
    List<unsigned> list;
    for (unsigned i = 0; i < 10; i++) { list.insertFront(i); }
